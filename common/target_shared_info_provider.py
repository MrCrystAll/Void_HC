...
class TargetSharedInfoProvider(SharedInfoProvider[Hashable, GameState]):
    ...
    def set_state(self, agents: List[Hashable], initial_state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        for agent in agents:
            shared_info[TARGET_HEADER][agent] = {}
        return shared_info
            
    def step(self, agents: List[Hashable], state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        return shared_info