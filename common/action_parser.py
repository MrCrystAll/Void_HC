...
class HCBotEnhancedActionParser(ActionParser[Hashable, np.ndarray, np.ndarray, GameState, tuple[str, int]]):
    ...
    def _make_lookup_table(self) -> np.ndarray:
        ...
        for atba_action in range(ATBAAction.N_ACTIONS):
            for flip_action in range(FlipAction.N_ACTIONS):
                actions.append(HCMachineAction(ATBAAction(atba_action), FlipAction(flip_action)))
    ...
    def parse_actions(self, actions: Dict[Hashable, np.ndarray], state: GameState, shared_info: Dict[str, Any]) -> Dict[Hashable, np.ndarray]:
