...
class FlipStateMachine(StateMachine[FlipState, FlipAction]):
    def reset(self, agents: list[Hashable], state: GameState, shared_info: dict[str, Any]):
        for agent in agents:
            self.states[agent] = FlipState.ON_GROUND if state.cars[agent].on_ground else FlipState.HAS_FLIPPED
    ...
    def step(self, actions: dict[Hashable, FlipAction], state: GameState, shared_info: dict[str, Any]):
