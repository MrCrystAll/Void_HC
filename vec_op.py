...
class OpVector3:
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            if key.start is not None:
                assert -2 <= key.start <= 2, (
                    f"Key slice start must be between -2 and 2, but {key.start} was given"
                )
    ...
    def __neg__(self) -> "OpVector3":
        return OpVector3(Vector3(*-self._data))
    ...
    def __isub__(self, other: "Vector3 | OpVector3 | int | float") -> Self:
        if isinstance(other, Vector3):
            value = Vector3(-other.x, -other.y, -other.z)
        else:
            value = -other
    ...
    def transposed(self) -> "OpVector3":
        return OpVector3.from_numpy(self._data.T)
    ...
    def from_numpy(numpy_array: np.ndarray) -> "OpVector3":
        assert numpy_array.shape == (3,), (
            f"Numpy array must be of shape (3, ), but received {numpy_array.shape}"
        )

        return OpVector3(Vector3(*numpy_array))
